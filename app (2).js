...
const STORAGE_KEY = 'escritor_48h_state_v5';
...
function updateTutorialUI() {
    ...
    if (target) {
        ...
        if (step.pos === 'bottom') {
            ...
            card.style.top = (rect.top + scrollY - 200) + 'px'; // Altura fixa aproximada para evitar corte
            card.style.left = (rect.left + rect.width/2 - 140) + 'px';
            arrow.className = 'tutorial-arrow down';
        }
        
        if (parseFloat(card.style.left) < 10) card.style.left = '10px';
        if (parseFloat(card.style.left) > window.innerWidth - 290) card.style.left = (window.innerWidth - 290) + 'px';
