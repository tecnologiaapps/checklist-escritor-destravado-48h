...
function loadState() {
    try {
        ...
        if (saved) {
            ...
            // Garantir que o estado carregado tenha a estrutura correta
...
function saveState() {
    try {
        ...
        // Backup imediato para garantir persistência
        console.log("Estado salvo no LocalStorage.");
        updateProgress();
    } catch (e) {
        console.error("Erro ao salvar estado no LocalStorage:", e);
        alert("Aviso: Não foi possível salvar seu progresso localmente. Verifique as permissões do seu navegador/app.");
    }
}
