...
const tutorialSteps = [
    { target: 'tut-prof', title: "Bem-vindo!", body: "Este app vai te ajudar a destravar sua escrita em 48h. Vamos te mostrar como ele funciona.", pos: 'bottom' },
    { target: 'tut-progress', title: "Seu Progresso", body: "Aqui você vê o Livro Mágico se preencher conforme você conclui os pontos do checklist.", pos: 'bottom' },
    { target: 'tut-btn-list', title: "Checklist", body: "Clique aqui para ver as 22 tarefas. Cada uma tem dicas exclusivas para sua história.", pos: 'top' },
    { target: 'tut-exports', title: "Exportação", body: "Salve seu progresso em TXT ou PDF completo (com textos e anotações) a qualquer momento.", pos: 'top' },
    { target: 'tut-backup', title: "Backup Seguro", body: "Faça um backup local para garantir que nunca perderá seus dados, mesmo trocando de celular.", pos: 'top' },
    { target: 'tut-social', title: "Redes Sociais", body: "Siga-me para aprender ainda mais sobre como escrever seu primeiro livro!", pos: 'top' }
];

const STORAGE_KEY = 'escritor_48h_state_v4';
...
// Listen for hardware back button (Android WebView)
...
// Custom Alert System (for APK compatibility)
function showAlert(title, body, confirmCb = null, timer = 0) {
    ...
    alertConfirmCallback = confirmCb;
...
function updateTutorialUI() {
    ...
    if (target) {
        ...
        if (step.pos === 'bottom') {
            card.style.top = (rect.bottom + scrollY + 15) + 'px';
            card.style.left = (rect.left + rect.width/2 - 140) + 'px';
            arrow.className = 'tutorial-arrow up';
        } else {
            card.style.top = (rect.top + scrollY - card.offsetHeight - 15) + 'px';
            card.style.left = (rect.left + rect.width/2 - 140) + 'px';
        ...
        if (cardRect.right > window.innerWidth - 10) card.style.left = (window.innerWidth - 290) + 'px';
...
function finishTutorial() {
    ...
    saveState();
    document.getElementById('tutorial-overlay').style.display = 'none';
    showAlert("Sucesso", "Tutorial concluído! Agora você está pronto para começar.");
}
...
function exportNotes() {
    ...
    showAlert("Exportado", "Anotações exportadas com sucesso! Verifique a pasta de Downloads do seu celular.");
}
...
async function exportPDF() {
    ...
    showAlert("PDF Gerado", "PDF gerado com sucesso! Verifique sua pasta de Downloads.");
}

function saveBackup() {
    const data = JSON.stringify(state);
    downloadFile(data, 'backup-escritor-48h.json', 'application/json');
    showAlert("Backup", "Seu backup está sendo salvo! Verifique sua pasta de Downloads e guarde este arquivo com segurança.");
}
...
function openFeedback() {
    showAlert("Feedback", "Após responder ao formulário, você receberá acesso a uma aula secreta para escritores!", () => {
        window.open(FEEDBACK_LINK, '_blank');
    });
}

function reportProblem() {
    const mailto = "mailto:senorlabios@senorlabios.com?subject=Problema no App Checklist Escritor";
...
function startResetTimer() {
    showAlert("Resetar Progresso", "Você tem certeza que deseja apagar TODO o seu progresso? Esta ação não pode ser desfeita.", () => {
        state = { checks: new Array(22).fill(false), notes: new Array(22).fill(""), tutorialDone: true, openPhases: [false, false, false], screenHistory: ['screen-dashboard'] };
        saveState();
        location.reload();
    }, 15);
}
...
function openExternal(e, url) {
    e.preventDefault();
    // In WebView, this triggers the system to open the URL in the external browser/app
