...
// State Management
const STORAGE_KEY = 'escritor_48h_state_v2';
...
function loadState() {
    try {
        ...
        if (saved) {
            ...
            if (parsed && Array.isArray(parsed.checks) && Array.isArray(parsed.notes)) {
                state = parsed;
            }
        }
    } catch (e) {
...
function togglePhase(phaseId) {
    ...
    if (header && content) {
        ...
        console.log(`Fase ${phaseId} alternada`);
    }
}
...
function renderList() {
    ...
    phases.forEach(phase => {
        ...
        phaseHeader.id = `phase-header-${phase.id}`;
        phaseHeader.onclick = () => togglePhase(phase.id);
        phaseHeader.innerHTML = `
            <div class="phase-header-content">
                <img src="${phase.icon}" alt="Fase" width="40" style="margin-right:10px;"> 
        ...
        phaseContent.id = `phase-content-${phase.id}`;
...
function showDetails(id) {
    ...
    container.innerHTML = `
        ...
        <textarea id="note-textarea-${id}" class="notes-area" placeholder="Minhas ideias para este ponto..." oninput="saveNote(${id}, this.value)">${note}</textarea>
    `;
...
function exportNotes() {
    try {
        ...
        alert("Anotações exportadas com sucesso!");
    } catch (e) {
        console.error("Erro ao exportar:", e);
        alert("Erro ao exportar anotações.");
    }
}
