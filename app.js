...
function handleBack() {
    if (state.screenHistory.length > 1) {
        ...
        // Se já no dashboard, recarrega para resetar estado
        if (window.AndroidBridge) {
            window.AndroidBridge.goToHome();
        } else {
            showScreen('screen-dashboard', false);
        }
        // Reset histórico para evitar loops
...
function exportNotes() {
    ...
    if (window.AndroidBridge) {
        alert("Tentando download TXT via AndroidBridge..."); // Debug: confirma chamada
...
async function exportPDF() {
    try {
        ...
        if (window.AndroidBridge) {
            alert("Tentando download PDF via AndroidBridge..."); // Debug: confirma chamada
